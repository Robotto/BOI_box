...
/*
  for (int i = 0; i < NUM_STICK_POSITIONS; i++) //0-3
  {
    stick_map[i]->EEPROM_ADDR=i;
  }
  for (int i = NUM_STICK_POSITIONS; i < NUM_STICK_POSITIONS+NUM_BUTTONS; i++) //4-7
  {
    button_map[i]->EEPROM_ADDR=i;
  }
  for (int i = NUM_STICK_POSITIONS+NUM_BUTTONS; i < NUM_STICK_POSITIONS+NUM_BUTTONS+NUM_COMBOS; i++) //8-11
  {
    combo_map[i]->EEPROM_ADDR=i;
  }
*/
...
void scan_inputs(void)
{
  for (int i = 0; i < NUM_STICK_POSITIONS; i++)
  {
    if(digitalRead(stick_map[i]->Leonardo_pin)==LOW)99 stick_map[i]->active=true; else stick_map[i]->active=false;
  }

  for (int i = 0; i < NUM_BUTTONS; i++)
  {
    if(digitalRead(button_map[i]->Leonardo_pin)==LOW) button_map[i]->active=true; else button_map[i]->active=false;
  }
}
