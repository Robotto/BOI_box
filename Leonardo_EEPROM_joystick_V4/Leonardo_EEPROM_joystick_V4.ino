...
#define DEBOUNCE_MS     20L      //milisecons to wait for inputs - debounce.
#define COMBO_FILTER_MS 50L      //miliseconds to wait for button - beause combos take precedence.
...
typedef struct //stick positions
{
  ...
  int Leonardo_pin;
  bool active;
  bool last_state;          //used for debouncing
...
//DONE: made a function called key_check where you can add bit patterns that will null the char read from prom and revert the binding to the default.
  ...
  if (key_check(temp_key_placeholder)) stick_map[i]->key_binding=temp_key_placeholder;
  }
  
  for (int i = 0; i < NUM_BUTTONS; i++) //key bindings for the buttons
  {
  temp_key_placeholder=(char)EEPROM.read(button_map[i]->EEPROM_ADDR);    
  if (key_check(temp_key_placeholder)) button_map[i]->key_binding=temp_key_placeholder;
  }

  for (int i = 0; i < NUM_COMBOS; i++) //key bindings for the combos
  {
  temp_key_placeholder=(char)EEPROM.read(combo_map[i]->EEPROM_ADDR);
  if (key_check(temp_key_placeholder)) combo_map[i]->key_binding=temp_key_placeholder;
  }
  ...
  delay(1000); //well.. that's just the way it is..
...
void scan_inputs(void)
{
  for (int i = 0; i < NUM_STICK_POSITIONS; i++)
  {
    if((digitalRead(stick_map[i]->Leonardo_pin)==LOW) && (stick_map[i]->last_state==false)) 
      {
        stick_map[i]->debounce_counter=millis();
        stick_map[i]->last_state=true;
      }
    else 
      {
        stick_map[i]->last_state=false;
        stick_map[i]->debounce_counter=0;
      }
    if ((millis()-(stick_map[i]->debounce_counter))>DEBOUNCE_MS)
    {
      stick_map[i]->active=true;
    }
    else stick_map[i]->active=false;
  }

  for (int i = 0; i < NUM_BUTTONS; i++)
  
  {
    if((digitalRead(button_map[i]->Leonardo_pin)==LOW) && (button_map[i]->last_state==false)) 
      {
        button_map[i]->debounce_counter=millis();
        button_map[i]->last_state=true;
      }
    else 
      {
        button_map[i]->last_state=false;
        button_map[i]->debounce_counter=0;
      }
    if ((millis()-button_map[i]->debounce_counter)>DEBOUNCE_MS)
    {
      button_map[i]->active=true;
    }
    else button_map[i]->active=false;
  }
}
    ...
    if(stick_map[i]->active && !(stick_map[i]->keypress_sent)) //is it active and has a keypress NOT been sent already
      {
        ...
        {
          if(bool_prog_mode==false) Keyboard.release(stick_map[i]->key_binding);
...
{
//handler:
  if(!global_combo_flag) return; //no need to run if no combos are active.
    ...
    if(combo_map[i]->active && !(combo_map[i]->keypress_sent)) //is it active and has a keypress NOT been sent already?
      {
        ...
        {
          if(bool_prog_mode==false) Keyboard.release(combo_map[i]->key_binding);
          combo_map[i]->keypress_sent=false; //remember that a key-release was sent
        }
  }
}

void handle_buttons(void) //if a button remains depressed for the duration of the COMBO_FILTER_MS miliseconds (default value is 50mS), send a keypress.
{
...
for (int i = 0; i < NUM_BUTTONS; i++)
  {
    if(button_map[i]->active && !(global_combo_flag)) //if the button is active AND no combos are active.
    {
      ...
      else if((millis() > button_map[i]->filter_counter + COMBO_FILTER_MS) && !(button_map[i]->keypress_sent))
      {
        if (bool_prog_mode==true) 
          {
      ...
      {
      if(bool_prog_mode==false) Keyboard.release(button_map[i]->key_binding);
...
bool key_check(char key)
{
  if ((key==0xFF) | (key==0x00)) return false; //ADD CHECKS HERE
  else return true;
}
