...
#define TEKKEN_FILTER_VALUE 50L  //miliseconds to wait for button
#define DEBOUNCE_MS     20L
...
bool global_combo_flag=false; //if set if a combo is active, to prevent single button presses to be registered.
...
void loop() //no interrupts, no args, no returns, no bullshit... just a bunch of polling loops.
{
  ...
  if (Serial.available()>0) 
  {
   ...
   if (incoming==4) set_prog_mode(false); //4 = EOT..
  }
  ...
  handle_buttons(); //if no combos, handle buttons, also delay them 50mS (about the time it takes to release a combo)
    ...
    if(stick_map[i]->active && !(stick_map[i]->keypress_sent)) //is it active and has a keypress NOT been sent already
      {
        if (bool_prog_mode==true) //PROGRAMMING MODE
          ...
          {
            if      (stick_map[i]->key_binding==left_remap)  Keyboard.press(KEY_LEFT_ARROW);
            else if (stick_map[i]->key_binding==right_remap) Keyboard.press(KEY_RIGHT_ARROW);
            else if (stick_map[i]->key_binding==up_remap)    Keyboard.press(KEY_UP_ARROW);
            else if (stick_map[i]->key_binding==down_remap)  Keyboard.press(KEY_DOWN_ARROW);
            else                Keyboard.press(stick_map[i]->key_binding);
          }
      ...
      else if (!(stick_map[i]->active) && (stick_map[i]->keypress_sent)) //if a key is no longer activated, but a keypress was sent before (because it was active during last check)
        {
          if(bool_prog_mode==false)
           {
            if      (stick_map[i]->key_binding==left_remap)  Keyboard.release(KEY_LEFT_ARROW);
            else if (stick_map[i]->key_binding==right_remap) Keyboard.release(KEY_RIGHT_ARROW);
            else if (stick_map[i]->key_binding==up_remap)    Keyboard.release(KEY_UP_ARROW);
            else if (stick_map[i]->key_binding==down_remap)  Keyboard.release(KEY_DOWN_ARROW);
            else                  Keyboard.release(stick_map[i]->key_binding);
           }
    ...
    if(combo_map[i]->active && !(combo_map[i]->keypress_sent)) //is it active and has a keypress NOT been sent already?
      {
        if (bool_prog_mode==true) 
          ...
          {
            if      (combo_map[i]->key_binding==left_remap)  Keyboard.press(KEY_LEFT_ARROW);
            else if (combo_map[i]->key_binding==right_remap) Keyboard.press(KEY_RIGHT_ARROW);
            else if (combo_map[i]->key_binding==up_remap)    Keyboard.press(KEY_UP_ARROW);
            else if (combo_map[i]->key_binding==down_remap)  Keyboard.press(KEY_DOWN_ARROW);
            else                Keyboard.press(combo_map[i]->key_binding);
          }
      ...
      else if (!(combo_map[i]->active) && (combo_map[i]->keypress_sent)) //if a key is no longer activated, but a keypress was sent before (because it was acitve during last check)?
        {
          if(bool_prog_mode==false)
           {
            if      (combo_map[i]->key_binding==left_remap)  Keyboard.release(KEY_LEFT_ARROW);
            else if (combo_map[i]->key_binding==right_remap) Keyboard.release(KEY_RIGHT_ARROW);
            else if (combo_map[i]->key_binding==up_remap)    Keyboard.release(KEY_UP_ARROW);
            else if (combo_map[i]->key_binding==down_remap)  Keyboard.release(KEY_DOWN_ARROW);
            else                  Keyboard.release(combo_map[i]->key_binding);
           }
    ...
    if(button_map[i]->active && !(global_combo_flag)) //if the button is active AND no combos are active.
    {
      ...
      else if((millis() > button_map[i]->filter_counter + TEKKEN_FILTER_VALUE) && !(button_map[i]->keypress_sent))
      {
        if (bool_prog_mode==true) 
          ...
          {
            if      (button_map[i]->key_binding==left_remap)  Keyboard.press(KEY_LEFT_ARROW);
            else if (button_map[i]->key_binding==right_remap) Keyboard.press(KEY_RIGHT_ARROW);
            else if (button_map[i]->key_binding==up_remap)    Keyboard.press(KEY_UP_ARROW);
            else if (button_map[i]->key_binding==down_remap)  Keyboard.press(KEY_DOWN_ARROW);
            else                Keyboard.press(button_map[i]->key_binding);
          }
      ...
      if(bool_prog_mode==false) 
        {
            if      (button_map[i]->key_binding==left_remap)  Keyboard.release(KEY_LEFT_ARROW);
            else if (button_map[i]->key_binding==right_remap) Keyboard.release(KEY_RIGHT_ARROW);
            else if (button_map[i]->key_binding==up_remap)    Keyboard.release(KEY_UP_ARROW);
            else if (button_map[i]->key_binding==down_remap)  Keyboard.release(KEY_DOWN_ARROW);
            else                  Keyboard.release(button_map[i]->key_binding);
        }
